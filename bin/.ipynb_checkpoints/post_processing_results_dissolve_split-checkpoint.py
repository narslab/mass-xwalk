...
def set_polygon_crs(polygon_df, road_df, intersection_df, dissolve_df):
    ...
    if polygon_df.crs != dissolve_df.crs:
        dissolve_df_transformed = dissolve_df.to_crs(polygon_df.crs)
    else:
        dissolve_df_transformed = dissolve_df.crs
        
    return(road_df_transformed, intersection_df_transformed, dissolve_df_transformed)
...
# Function to map gridcode to class
def map_gridcode_to_class(grid_code):
    if grid_code == 1:
        return 'continental'
    elif grid_code == 2:
        return 'parallel'
...
def main(): 
    ...
    roads_arc_transformed_west21, intersection_pts_transformed_west21, dissolve_split_west21 = set_polygon_crs(wm21_filtered, roads_arc, at_grade_intersections, dissolve_split)
    roads_arc_transformed_east21, intersection_pts_transformed_east21, dissolve_split_east21 = set_polygon_crs(em21_filtered, roads_arc, at_grade_intersections, dissolve_split)
    roads_arc_transformed_west19, intersection_pts_transformed_west19, dissolve_split_west19 = set_polygon_crs(wm19_filtered, roads_arc, at_grade_intersections, dissolve_split)
    roads_arc_transformed_east19, intersection_pts_transformed_east19, dissolve_split_east19 = set_polygon_crs(em19_filtered, roads_arc, at_grade_intersections, dissolve_split)
    ...
    start = time.time()
    wm21_intersection_midblock = midblock_test(wm21_intersection, dissolve_split_west21, 8)
    em21_intersection_midblock = midblock_test(em21_intersection, dissolve_split_east21, 8)
    wm19_intersection_midblock = midblock_test(wm19_intersection, dissolve_split_west19, 8)
    em19_intersection_midblock = midblock_test(em19_intersection, dissolve_split_east19, 8)
    elapsed = time.time() - start
...
#     em19_count_final_filtered = em19_count_final[~((em19_count_final['type'] == 'other') | (em19_count_final['type'] == 'parking') | (em19_count_final['category'] == 'fp'))]
    ...
    output_dir = f'results/{today_date}/dissolve_split/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)
    # Save GeoDataFrame to csv
    wm21_filename = f"{wm21_count_final.iloc[0]['source']}_{wm21_count_final.iloc[0]['year']}_dissolve_split.csv"
    wm19_filename = f"{wm19_count_final.iloc[0]['source']}_{wm19_count_final.iloc[0]['year']}_dissolve_split.csv"
    em21_filename = f"{em21_count_final.iloc[0]['source']}_{em21_count_final.iloc[0]['year']}_dissolve_split.csv"
    em19_filename = f"{em19_count_final.iloc[0]['source']}_{em19_count_final.iloc[0]['year']}_dissolve_split.csv"
